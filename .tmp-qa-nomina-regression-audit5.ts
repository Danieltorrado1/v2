...
process.env.ENV_FILE = process.env.ENV_FILE?.trim() || '.env.qa';
...
async function main() {
  ...
  if (!user) {
    throw new Error('QA TALENTO HUMANO user id 10 not found in QA DB');
  }
  ...
  if (!period) {
    throw new Error('periodo_id 2 not found in QA DB');
  }
    ...
    const auditedRequests = [
      {
        ...
        endpoint: `/nomina/periodos?empresa_id=${period.empresa_id}&page=1&limit=200`,
        method: 'GET',
        permission: 'nomina.operativa.read|nomina.read',
      },
      {
        component: 'NominaPage.loadTiposNovedad',
        endpoint: `/nomina/tipos-novedad?empresa_id=${period.empresa_id}`,
        method: 'GET',
        permission: 'nomina.operativa.read|nomina.read',
      },
      {
      ...
      },
      {
        ...
        endpoint: `/nomina/periodos/2/empleados?empresa_id=${period.empresa_id}&page=1&limit=200`,
        method: 'GET',
      ...
      },
      {
        ...
        endpoint: `/nomina/periodos/2/empleados-operativos?empresa_id=${period.empresa_id}&page=1&limit=200`,
        method: 'GET',
      ...
      },
      {
        ...
        endpoint: `/nomina/cobertura/externos-operativos?periodo_id=2&empresa_id=${period.empresa_id}`,
        method: 'GET',
        permission: 'nomina.operativa.read',
      },
    ] as const;
