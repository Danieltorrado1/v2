...
const PERMISSIONS = [
  {
    modulo: 'auditoria',
    accion: 'read',
    descripcion: 'Permiso base para auditoria.read'
  },
  {
    modulo: 'auditoria',
    accion: 'export',
    descripcion: 'Permiso base para auditoria.export'
  }
] as const;
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Audit permissions seeded successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Audit permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
