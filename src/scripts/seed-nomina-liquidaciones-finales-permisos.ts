...
const PERMISSIONS = [
  {
    modulo: 'nomina.liquidaciones_finales',
    accion: 'read',
    descripcion: 'Permiso base para nomina.liquidaciones_finales.read'
  },
  {
    modulo: 'nomina.liquidaciones_finales',
    accion: 'write',
    descripcion: 'Permiso base para nomina.liquidaciones_finales.write'
  },
  {
    modulo: 'nomina.liquidaciones_finales',
    accion: 'dashboard',
    descripcion: 'Permiso base para nomina.liquidaciones_finales.dashboard'
  },
  {
    modulo: 'nomina.liquidaciones_finales',
    accion: 'alertas',
    descripcion: 'Permiso base para nomina.liquidaciones_finales.alertas'
  }
] as const;
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Nomina liquidaciones finales permissions seeded successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Nomina liquidaciones finales permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
