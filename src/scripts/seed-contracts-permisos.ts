...
const PERMISOS = [
  ['contracts', 'read', 'Permiso para consultar contratos en el motor contractual'],
  ['contracts', 'create', 'Permiso para crear contratos en el motor contractual'],
  ['contracts', 'update', 'Permiso para actualizar contratos en el motor contractual'],
  ['contracts.status', 'update', 'Permiso para actualizar estados sensibles del contrato'],
  ['contracts.events', 'read', 'Permiso para consultar eventos contractuales'],
  ['contracts.events', 'create', 'Permiso para registrar y anular eventos contractuales'],
  ['contracts.documents', 'read', 'Permiso para consultar expediente documental contractual'],
  ['contracts.documents', 'upload', 'Permiso para cargar documentos contractuales'],
  ['contracts.documents', 'review', 'Permiso para enviar a revision, aprobar, devolver o anular documentos contractuales'],
  ['contracts.documents', 'download', 'Permiso para descargar documentos contractuales'],
  ['contracts.exceptions', 'read', 'Permiso para consultar excepciones documentales contractuales'],
  ['contracts.exceptions', 'create', 'Permiso para crear excepciones documentales contractuales'],
  ['contracts.exceptions', 'resolve', 'Permiso para regularizar o revocar excepciones documentales contractuales']
] as const;
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Contracts permissions seeded successfully.');
    for (const [roleName, permissionCodes] of Object.entries(ROLE_ASSIGNMENTS)) {
      const assigned = roles.has(roleName) ? permissionCodes.join(', ') : 'role not present';
      console.log(`${roleName}: ${assigned}`);
    }
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Contracts permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
