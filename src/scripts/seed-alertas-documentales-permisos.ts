...
const PERMISSIONS = [
  {
    modulo: 'alertas.documentales',
    accion: 'read',
    descripcion: 'Permiso base para alertas.documentales.read'
  },
  {
    modulo: 'alertas.documentales',
    accion: 'generate',
    descripcion: 'Permiso base para alertas.documentales.generate'
  },
  {
    modulo: 'alertas.documentales',
    accion: 'update',
    descripcion: 'Permiso base para alertas.documentales.update'
  },
  {
    modulo: 'dashboard.saas',
    accion: 'read',
    descripcion: 'Permiso base para dashboard.saas.read'
  }
] as const;
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Alertas documentales permissions seeded successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Alertas documentales permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
