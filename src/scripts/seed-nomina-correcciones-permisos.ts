...
const NOMINA_CORRECCION_PERMISSION_DEFINITIONS = [
  {
...
const seedPermissions = async (client: PoolClient): Promise<Map<string, string>> => {
  ...
  for (const permission of NOMINA_CORRECCION_PERMISSION_DEFINITIONS) {
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Nomina corrections permissions seeded successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Nomina corrections permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
