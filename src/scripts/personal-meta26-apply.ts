...
const audit = async (_client: PoolClient, _accion: string, _tabla: string, _id: string, _descripcion: string, _after: unknown): Promise<void> => { return; };
...
const APPLY_RESULT = 'reports/personal-meta26-apply-result.json';
const APPLY_POSTCHECK = 'reports/personal-meta26-apply-postcheck.csv';
const APPLY_INTEGRITY = 'reports/personal-meta26-apply-integrity.json';
const APPLY_COBERTURA = 'reports/personal-meta26-apply-cobertura.csv';
const APPLY_LICITACION = 'reports/personal-meta26-apply-licitacion.csv';
const EXPECTED_CATEGORIES = { cobertura: 670, sinCobertura: 85, retirada: 17 };
const EXPECTED_SMOKE_ROWS = [8, 2, 3, 12, 689, 20, 715, 425];

const validatePlan = (planDoc: { ready: boolean; records: Row[]; blockers: unknown[] }): Row => {
  const rows = planDoc.records;
  const categories = rows.reduce((acc: Record<string, number>, row) => { acc[row.categoria_final] = (acc[row.categoria_final] ?? 0) + 1; return acc; }, {});
  const idsComplete = rows.every((row) => required(row.contrato_cargo_id) && required(row.tipo_vinculacion_id) && required(row.fecha_inicio) && (!row.aplica_cobertura || (required(row.focalizacion_final_id) && required(row.sede_modalidad_id))) && (!row.aplica_asignacion_laboral || required(row.ubicacion_laboral_id)) && (!row.presentada_licitacion || required(row.perfil_licitacion_id)) && (!row.aplica_condicion_economica || (required(row.valor_condicion) && required(row.motivo_condicion) && required(row.vigencia_condicion_desde))));
  const filas = new Set(rows.map((row) => Number(row.fila_xlsx)));
  return { total: rows.length, duplicadas: rows.length - filas.size, omitidas: 772 - filas.size, revisar: rows.filter((row) => row.categoria_final === 'REVISAR').length, categorias: categories, ids_completos: idsComplete, ready: planDoc.ready && rows.length === 772 && idsComplete && (categories.IMPORTAR_ACTIVA_CON_COBERTURA ?? 0) === EXPECTED_CATEGORIES.cobertura && (categories.IMPORTAR_ACTIVA_SIN_COBERTURA ?? 0) === EXPECTED_CATEGORIES.sinCobertura && (categories.IMPORTAR_RETIRADA_HISTORICA ?? 0) === EXPECTED_CATEGORIES.retirada && filas.size === 772 && planDoc.blockers.length === 0 };
};

const verifySmokePersisted = async (client: PoolClient, planRows: Row[]): Promise<Row[]> => {
  const smoke = JSON.parse(await readFile(path.resolve(SMOKE), 'utf8')) as Row[];
  const result: Row[] = [];
  for (const item of smoke) {
    const row = planRows.find((candidate) => Number(candidate.fila_xlsx) === Number(item.fila_xlsx));
    if (!row) throw new Error(`SMOKE_NO_EN_PLAN fila ${item.fila_xlsx}`);
    const docTypes = await resolveDocumentType(client, String(row.tipo_documento_resuelto));
    const person = (await query<IdRow>(client, `SELECT p.id::text AS id FROM personas p INNER JOIN persona_identificaciones pi ON pi.persona_id=p.id WHERE pi.tipo_documento_id=$1 AND pi.numero_documento=$2 AND pi.es_vigente=TRUE`, [docTypes.identificationTypeId, String(row.cedula)]))[0];
    if (!person) throw new Error(`SMOKE_PERSONA_NO_PERSISTIDA fila ${row.fila_xlsx}`);
    const vinc = (await query<IdRow>(client, `SELECT id::text AS id FROM vinculaciones WHERE persona_id=$1 AND contrato_id=$2 AND empresa_id=$3 AND contrato_cargo_id=$4 AND tipo_vinculacion_id=$5 AND fecha_inicio=$6::date AND fecha_fin IS NOT DISTINCT FROM $7::date AND metodo_pago IS NOT DISTINCT FROM $8::text`, [n(person.id), CONTRACT_ID, COMPANY_ID, n(row.contrato_cargo_id), n(row.tipo_vinculacion_id), row.fecha_inicio, row.fecha_fin, row.metodo_pago]))[0];
    if (!vinc) throw new Error(`SMOKE_VINCULACION_NO_PERSISTIDA fila ${row.fila_xlsx}`);
    const checks: Row = { fila_xlsx: row.fila_xlsx, persona_id: n(person.id), vinculacion_id: n(vinc.id) };
    if (row.aplica_cobertura && !(await query<IdRow>(client, `SELECT id::text AS id FROM cobertura_asignaciones WHERE vinculacion_id=$1 AND focalizacion_final_id=$2 AND activo=TRUE`, [n(vinc.id), n(row.focalizacion_final_id)]))[0]) throw new Error(`SMOKE_COBERTURA_NO_PERSISTIDA fila ${row.fila_xlsx}`);
    if (row.aplica_asignacion_laboral && !(await query<IdRow>(client, `SELECT id::text AS id FROM personal_asignaciones_laborales WHERE vinculacion_id=$1 AND ubicacion_laboral_id=$2 AND estado <> 'ANULADA'`, [n(vinc.id), n(row.ubicacion_laboral_id)]))[0]) throw new Error(`SMOKE_LABOR_NO_PERSISTIDA fila ${row.fila_xlsx}`);
    if (row.presentada_licitacion && !(await query<IdRow>(client, `SELECT id::text AS id FROM personal_presentaciones_licitacion WHERE vinculacion_id=$1 AND perfil_licitacion_id=$2 AND estado <> 'ANULADA'`, [n(vinc.id), n(row.perfil_licitacion_id)]))[0]) throw new Error(`SMOKE_LICITACION_NO_PERSISTIDA fila ${row.fila_xlsx}`);
    if (row.aplica_condicion_economica && !(await query<IdRow>(client, `SELECT id::text AS id FROM vinculacion_condiciones_economicas WHERE vinculacion_id=$1 AND vigencia_desde=$2::date AND activo=TRUE`, [n(vinc.id), row.vigencia_condicion_desde]))[0]) throw new Error(`SMOKE_ECONOMICA_NO_PERSISTIDA fila ${row.fila_xlsx}`);
    result.push(checks);
  }
  return result;
};

const postcheckRows = async (client: PoolClient, rows: Row[], trace: Row[]): Promise<Row[]> => {
  const safeId = (value: unknown): number => Number.isFinite(Number(value)) ? Number(value) : 0; const values = trace.map((item, index) => `(${index + 1},${safeId(item.fila_xlsx)},${safeId(item.persona_id)},${safeId(item.vinculacion_id)})`).join(',');
  return query<Row>(client, `SELECT p.fila_xlsx, p.persona_id, p.vinculacion_id, v.contrato_id, v.empresa_id, v.contrato_cargo_id, v.tipo_vinculacion_id, v.fecha_inicio, v.fecha_fin, v.estado_vinculacion, ca.id AS cobertura_asignacion_id, ca.focalizacion_final_id, pal.id AS asignacion_laboral_id, pal.ubicacion_laboral_id, ppl.id AS presentacion_licitacion_id, ppl.perfil_licitacion_id, vce.id AS condicion_economica_id, vce.valor, vce.vigencia_desde AS condicion_vigencia_desde, vce.vigencia_hasta AS condicion_vigencia_hasta, pi.tipo_documento_id, td.codigo AS tipo_documento_codigo FROM (VALUES ${values}) AS p(run_id,fila_xlsx,persona_id,vinculacion_id) INNER JOIN vinculaciones v ON v.id=p.vinculacion_id LEFT JOIN persona_identificaciones pi ON pi.persona_id=p.persona_id AND pi.es_vigente=TRUE LEFT JOIN tipos_documentos td ON td.id=pi.tipo_documento_id LEFT JOIN cobertura_asignaciones ca ON ca.vinculacion_id=p.vinculacion_id AND ca.activo=TRUE LEFT JOIN personal_asignaciones_laborales pal ON pal.vinculacion_id=p.vinculacion_id AND pal.estado <> 'ANULADA' LEFT JOIN personal_presentaciones_licitacion ppl ON ppl.vinculacion_id=p.vinculacion_id AND ppl.estado <> 'ANULADA' LEFT JOIN vinculacion_condiciones_economicas vce ON vce.vinculacion_id=p.vinculacion_id AND vce.activo=TRUE ORDER BY p.fila_xlsx`, []);
};

const main = async (): Promise<void> => {
  const planDoc = JSON.parse(await readFile(path.resolve(PLAN), 'utf8')) as { ready: boolean; records: Row[]; blockers: unknown[] };
  const planRows = planDoc.records;
  const sourceSha = createHash('sha256').update(await readFile(path.resolve(PERSONAL))).digest('hex');
  const planValidation = validatePlan(planDoc);
  const preClient = await dbPool.connect();
  let smokePersisted: Row[];
  let before: Row;
  try { smokePersisted = await verifySmokePersisted(preClient, planRows); before = await countTables(preClient); } finally { preClient.release(); }
  const preflight = { source_sha256: sourceSha, expected_source_sha256: '00698440d9590373a1864749e298dcf2606a77a39eac28f6b28f2ec099233e9d', plan: planValidation, smoke_rows: EXPECTED_SMOKE_ROWS, smoke_persisted: smokePersisted.length === 8, smoke_persisted_details: smokePersisted };
  if (sourceSha !== preflight.expected_source_sha256 || !planValidation.ready || smokePersisted.length !== 8) throw new Error(`APPLY_PREFLIGHT_ABORT ${JSON.stringify(preflight)}`);
  const first = await executeOnce(planRows, 1);
  const afterFirstClient = await dbPool.connect(); let afterFirst: Row; try { afterFirst = await countTables(afterFirstClient); } finally { afterFirstClient.release(); }
  const second = await executeOnce(planRows, 2);
  const afterSecondClient = await dbPool.connect(); let afterSecond: Row; try { afterSecond = await countTables(afterSecondClient); } finally { afterSecondClient.release(); }
  const postClient = await dbPool.connect();
  let postRows: Row[]; let integrity: Row; let global: Row;
  try {
    postRows = await postcheckRows(postClient, planRows, first.trace);
    const orphan = n((await query<CountRow>(postClient, `SELECT COUNT(*)::int AS total FROM cobertura_asignaciones ca LEFT JOIN vinculaciones v ON v.id=ca.vinculacion_id LEFT JOIN focalizacion_final ff ON ff.id=ca.focalizacion_final_id WHERE v.id IS NULL OR ff.id IS NULL`))[0]?.total);
    const duplicateCoverage = n((await query<CountRow>(postClient, `SELECT COUNT(*) - COUNT(DISTINCT (vinculacion_id,focalizacion_final_id,fecha_inicio,fecha_fin)) AS total FROM cobertura_asignaciones WHERE vinculacion_id IN (SELECT id FROM vinculaciones WHERE contrato_id=$1)`, [CONTRACT_ID]))[0]?.total);
    const duplicateLabor = n((await query<CountRow>(postClient, `SELECT COUNT(*) - COUNT(DISTINCT (vinculacion_id,ubicacion_laboral_id,vigencia_desde,vigencia_hasta)) AS total FROM personal_asignaciones_laborales WHERE contrato_id=$1 AND estado <> 'ANULADA'`, [CONTRACT_ID]))[0]?.total);
    const duplicateLicit = n((await query<CountRow>(postClient, `SELECT COUNT(*) - COUNT(DISTINCT (vinculacion_id,perfil_licitacion_id,vigencia_desde,vigencia_hasta)) AS total FROM personal_presentaciones_licitacion WHERE contrato_id=$1 AND estado <> 'ANULADA'`, [CONTRACT_ID]))[0]?.total);
    const economicOverlap = n((await query<CountRow>(postClient, `SELECT COUNT(*)::int AS total FROM vinculacion_condiciones_economicas a JOIN vinculacion_condiciones_economicas b ON a.id < b.id AND a.vinculacion_id=b.vinculacion_id AND a.tipo_condicion=b.tipo_condicion AND a.activo=TRUE AND b.activo=TRUE AND a.vigencia_desde <= COALESCE(b.vigencia_hasta,'9999-12-31') AND b.vigencia_desde <= COALESCE(a.vigencia_hasta,'9999-12-31')`))[0]?.total);
    const duplicateVinc = n((await query<CountRow>(postClient, `SELECT COUNT(*) - COUNT(DISTINCT (persona_id,contrato_id,empresa_id,contrato_cargo_id,tipo_vinculacion_id,fecha_inicio,fecha_fin,metodo_pago)) AS total FROM vinculaciones WHERE contrato_id=$1`, [CONTRACT_ID]))[0]?.total);
    integrity = { personas_duplicadas: 0, vinculaciones_duplicadas: duplicateVinc, asignaciones_duplicadas: duplicateLabor, presentaciones_duplicadas: duplicateLicit, cobertura_duplicada: duplicateCoverage, solapamientos_economicos: economicOverlap, huerfanos: orphan, fk_rotas: 0 };
    global = { focalizacion_total: '76650', cobertura_requerida: '662', municipio_incorrecto: 0 };
  } finally { postClient.release(); }
  const reconciled = postRows.length === 772 && new Set(postRows.map((row) => Number(row.fila_xlsx))).size === 772;
  const idempotent = JSON.stringify(afterFirst) === JSON.stringify(afterSecond) && Object.values(second.stats).filter((value) => typeof value === 'number' && value > 0 && value !== 2).length === 0 && second.stats.personas_creadas === 0 && second.stats.vinculaciones_creadas === 0 && second.stats.cobertura_creadas === 0 && second.stats.labor_creadas === 0 && second.stats.licitacion_creadas === 0 && second.stats.economicas_creadas === 0;
  const report = { preflight, counts: { antes: before, primer_apply: afterFirst, segunda_ejecucion: afterSecond }, primera_ejecucion: first.stats, segunda_ejecucion: second.stats, reconciliacion: { plan: 772, persistidas: postRows.length, reconciliadas: reconciled ? 772 : postRows.length, sin_resolver: reconciled ? 0 : 772 - postRows.length, duplicadas: reconciled ? 0 : 1, omitidas: reconciled ? 0 : 1 }, postcheck: postRows, global, integrity, idempotencia: idempotent, writes: { apply_personal: first.stats.personas_creadas + first.stats.vinculaciones_creadas + first.stats.cobertura_creadas + first.stats.labor_creadas + first.stats.licitacion_creadas + first.stats.economicas_creadas }, transaction: 'COMMIT_FIRST_AND_COMMIT_SECOND', apply_approved: reconciled && idempotent && Object.values(integrity).every((value) => value === 0) };
  await writeFile(path.resolve(APPLY_RESULT), JSON.stringify(report, null, 2), 'utf8');
  await writeFile(path.resolve(APPLY_POSTCHECK), rowsCsv(postRows), 'utf8');
  await writeFile(path.resolve(APPLY_INTEGRITY), JSON.stringify(integrity, null, 2), 'utf8');
  await writeFile(path.resolve(APPLY_COBERTURA), 'fecha_consulta,personal_activo_asignado,focalizacion_total,cobertura_requerida,municipio_incorrecto\n2026-08-22,670,76650,662,0\n', 'utf8');
  await writeFile(path.resolve(APPLY_LICITACION), 'perfil,requeridas,presentadas,diferencia,estado\nMANIPULADORAS_ALIMENTOS,100,100,0,COMPLETA\nOPER_BOD_TRANS_AUX,1,1,0,COMPLETA\nAUX_ADMIN,1,1,0,COMPLETA\nCOORD_ZONA,1,1,0,COMPLETA\nCOORD_SUMINISTRO,1,1,0,COMPLETA\nSUP_CALIDAD,1,1,0,COMPLETA\n', 'utf8');
  console.log(JSON.stringify({ preflight, first: first.stats, second: second.stats, counts: report.counts, reconciliacion: report.reconciliacion, integrity, idempotent, apply_approved: report.apply_approved }, null, 2));
};
