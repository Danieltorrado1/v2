...
const PERMISSIONS = [
  {
    modulo: 'nomina.vacaciones',
    accion: 'read',
    descripcion: 'Permiso base para nomina.vacaciones.read'
  },
  {
    modulo: 'nomina.vacaciones',
    accion: 'write',
    descripcion: 'Permiso base para nomina.vacaciones.write'
  },
  {
    modulo: 'nomina.vacaciones',
    accion: 'dashboard',
    descripcion: 'Permiso base para nomina.vacaciones.dashboard'
  },
  {
    modulo: 'nomina.vacaciones',
    accion: 'alertas',
    descripcion: 'Permiso base para nomina.vacaciones.alertas'
  }
] as const;
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Nomina vacations permissions seeded successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Nomina vacations permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
