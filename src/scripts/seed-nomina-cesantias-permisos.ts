...
const PERMISSIONS = [
  {
    modulo: 'nomina.cesantias',
    accion: 'read',
    descripcion: 'Permiso base para nomina.cesantias.read'
  },
  {
    modulo: 'nomina.cesantias',
    accion: 'write',
    descripcion: 'Permiso base para nomina.cesantias.write'
  },
  {
    modulo: 'nomina.cesantias',
    accion: 'dashboard',
    descripcion: 'Permiso base para nomina.cesantias.dashboard'
  },
  {
    modulo: 'nomina.cesantias',
    accion: 'alertas',
    descripcion: 'Permiso base para nomina.cesantias.alertas'
  }
] as const;
...
const main = async (): Promise<void> => {
  ...
  try {
    ...
    console.log('Nomina cesantias permissions seeded successfully.');
  } catch (error) {
    await client.query('ROLLBACK');
    console.error('Nomina cesantias permissions seed failed.');
    console.error(error);
    process.exitCode = 1;
  } finally {
    client.release();
