...
test('la sincronizacion cierra la brecha economica de materializacion tardia', async () => {
  const source = (await readFile('src/modules/nomina/infrastructure/repositories/nomina-poblacion.repository.ts', 'utf8'))
    + (await readFile('src/modules/nomina/application/nomina-poblacion.service.ts', 'utf8'))
    + (await readFile('src/modules/nomina/nomina.service.ts', 'utf8'));
  ...
  assert.match(source, /new Set\(recalculableEmployeeIds\)/);
