...
const syntheticTables = [
  ...
  'cobertura_externos', 'auditoria_eventos'
];
...
async function main() {
  ...
  nomina_liquidaciones: ['requiere_recalculo', 'total_liquidacion', 'deducciones', 'salario_base'],
      nomina_movimientos: ['familia_movimiento', 'tipo_movimiento', 'valor_total'],
    ...
    await q(client, `INSERT INTO nomina_tipos_novedad(id,codigo_operativo,nombre,categoria,modelo_registro,activo,permite_rango) VALUES ($1,'SINT','Novedad sintética','AUSENCIA','ORDINARIA',TRUE,TRUE) ON CONFLICT DO NOTHING`, [ids.typeNovelty]);
    ...
    console.log(`E2E Phase 3: OK (${eventCounter} eventos sintéticos)`);
  } finally {
    try {
      await q(client, `DELETE FROM integracion_evento_impactos WHERE evento_id IN (SELECT id FROM integracion_eventos WHERE idempotency_key LIKE 'phase53-%')`);
      await q(client, `DELETE FROM integracion_eventos WHERE idempotency_key LIKE 'phase53-%'`);
