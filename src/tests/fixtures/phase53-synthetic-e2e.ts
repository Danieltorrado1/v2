...
async function main() {
  ...
  nomina_liquidaciones: ['requiere_recalculo', 'total_liquidacion', 'deducciones', 'salario_base'],
    ...
    assert.equal(pnrImpact.rows[0]?.recalc_estado, 'RECALCULADA');
    const pnrOutput = await q(client, `SELECT COUNT(*)::int AS count FROM integracion_eventos WHERE idempotency_key=$1`, [`integracion-recalc:${pnrEvent}:${ids.vincA}:${ids.openA}`]); assert.equal(Number(pnrOutput.rows[0].count), 1);
    ...
    assert.equal(Number(turnAfterCreate.rows[0].neto) - Number(turnBefore.rows[0].neto), 500);
    ...
    const turnAfterUpdate = await q(client, `SELECT neto_pagar::numeric AS neto FROM nomina_empleados WHERE id=$1`, [ids.employeeOpen]); assert.equal(Number(turnAfterUpdate.rows[0].neto) - Number(turnBefore.rows[0].neto), 700);
