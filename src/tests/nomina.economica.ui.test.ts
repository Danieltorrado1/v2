...
const procesosUi = readFileSync('FrontendNuevo/src/pages/admin/ConfiguracionGeneral/tabs/NominaProcesosTab.tsx', 'utf8');
...
test('tarifas de turnos exponen subtab y formulario por contrato y modalidad', () => {
  for (const token of [
    'Tarifas de turnos',
    'TURNO_INTERNO',
    'TURNO_EXTERNO',
    ...
    '/turn-shift-rates',
    'No fue posible cargar las modalidades del contrato seleccionado.',
  ]) {
    assert.ok(ui.includes(token), token);
  }

  assert.match(procesosUi, /TARIFAS DE TURNOS/);
  assert.match(procesosUi, /tab === 'tarifas-turnos'/);
  assert.match(procesosUi, /<TurnShiftRatesTab \/>/);
});

test('tarifas de turnos validan modalidad activa del contrato y preservan la actual al editar', () => {
  assert.match(
    service,
    /FROM focalizacion_final ff[\s\S]*WHERE ff\.contrato_id = \$1::bigint[\s\S]*AND ff\.modalidad_id = \$2::bigint[\s\S]*AND ff\.activo = TRUE/
  );
  assert.match(ui, /const availableModalities = useMemo\(/);
  assert.match(ui, /currentModalidad\.nombre\?\.trim\(\)/);
});
