...
export async function listInstituciones(query: InstitucionesQuery, tenant: TenantAccessContext) {
  ...
  const periods = (await dbQuery<Row>(`SELECT DISTINCT fv.id::text id,INITCAP(TO_CHAR(fv.vigente_desde,'TMMonth YYYY')) nombre,EXTRACT(YEAR FROM fv.vigente_desde)::int anio,EXTRACT(MONTH FROM fv.vigente_desde)::int mes ${source} WHERE ${tenantScope} AND ff.contrato_id=$${contractP}::bigint ORDER BY anio DESC,mes DESC`, params)).rows;
  const managers = gestorTable.size ? (await dbQuery<Row>(`SELECT DISTINCT gestor.id,gestor.nombre ${select.slice(select.indexOf(source))} WHERE gestor.id IS NOT NULL ORDER BY gestor.nombre`, params)).rows : [];
