...
import { setEmpresaActiveState } from '../configuracion/configuracion.admin.service';
import { changeCompanyPlan, clearCompanyOverride, getCompanySaasHistory, getEmpresaCapabilities, getGlobalCompanyUsage, listCompanySaasSummaries, listGlobalCompanyControl, listModules, listPlans, savePlan, setCompanyOverride } from './saas.service';
...
router.get('/companies/:empresaId/usage',asyncHandler(async(req,res)=>{globalAdmin(req);res.json({data:await getGlobalCompanyUsage(id.parse(req.params.empresaId),req.tenant!)});}));
router.patch('/companies/:empresaId/status',asyncHandler(async(req,res)=>{globalAdmin(req);const empresaId=id.parse(req.params.empresaId);const input=companyStatusSchema.parse(req.body);const audit=getAuditRequestMeta(req);res.json({data:await setEmpresaActiveState(empresaId,input.activo,{userId:String(req.user!.userId),ip:audit.ip??null,userAgent:audit.user_agent??null},input.observacion,req.tenant)});}));
