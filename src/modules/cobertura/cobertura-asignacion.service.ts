...
export const persistCanonicalAssignmentVersion = async (client: PoolClient, input: { contratoId: string | number; vinculacionId: string | number; fechaDesde: string; contexto: CanonicalAssignmentContext; actor: string; motivo: string }): Promise<void> => {
  ...
  if (existing && String(existing.fecha_inicio).slice(0, 10) < input.fechaDesde) {
    ...
    await client.query(`UPDATE cobertura_asignaciones SET focalizacion_final_id=(SELECT ff.id FROM focalizacion_final ff WHERE ff.contrato_id=$8::bigint AND ff.municipio_id=$3::bigint AND ff.institucion_id=$9::bigint AND ff.sede_id=$10::bigint AND ff.modalidad_id=$11::bigint AND COALESCE(ff.activo,TRUE) ORDER BY ff.id DESC LIMIT 1),municipio_id=$3::bigint,institucion=$4,sede=$5,modalidad=$6,observacion=$7 WHERE id=$1::bigint`, [existing.id, input.contexto.cobertura_asignacion_id ?? null, input.contexto.municipio_id ?? null, input.contexto.institucion ?? null, input.contexto.sede ?? null, input.contexto.modalidad ?? null, input.motivo, input.contratoId, input.contexto.institucion_id ?? null, input.contexto.sede_id ?? null, input.contexto.modalidad_id ?? null]);
    return;
  }
