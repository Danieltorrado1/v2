...
export default function GlobalAdminPage() {
  ...
  useEffect(() => { if (isGlobalAdministrator(user)) void getGlobalAdminDashboard().then(setData).catch((e) => setError(e instanceof Error ? e.message : 'No fue posible cargar el dashboard global.')); }, [user]);
  if (!isGlobalAdministrator(user)) return <div className="adm-notice warning"><AlertTriangle size={14} /> Acceso exclusivo para ADMINISTRADOR global.</div>;
  const k = data?.kpis;
  const cards = [['Empresas totales', k?.empresas_total, Building2], ['Empresas activas', k?.empresas_activas, Building2], ['Usuarios activos', k?.usuarios_activos, Users], ['Personas activas', k?.personal_total, UserRoundCheck], ['Contratos activos', k?.contratos_activos, BarChart3], ['Módulos activos', k?.modulos_activos, Boxes]] as const;
