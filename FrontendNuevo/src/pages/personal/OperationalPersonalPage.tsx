        ...
        {!contratoId ? <div className="op-empty">Selecciona un contrato para abrir la vista operativa.</div> : tableError ? <div className="op-state error"><AlertTriangle size={16} />{tableError}</div> : tableLoading && !tableData ? <div className="op-empty">Cargando personal del contrato...</div> : <>
          <div className="op-table-scroll"><table className="op-table"><thead><tr><th className="is-worker">Trabajador</th><th className="is-role">Cargo</th><th className="is-assignment">Asignación</th><th className="is-modality">Modalidad</th><th className="is-offer">Oferta</th><th className="is-status">Estado</th><th className="is-date">Ingreso</th><th className="is-updated">Última actualización</th><th className="is-action">Expediente</th><th className="is-menu">Acción</th></tr></thead><tbody>
            ...
            {(tableData?.items ?? []).map((item) => <tr key={item.vinculacion_id} className={item.vinculacion_id === selectedVinculacionId ? "is-selected" : ""} onClick={() => setSelectedVinculacionId(item.vinculacion_id)}>
              <td><div className="op-worker-cell"><span className="op-avatar" aria-hidden="true">{getInitials(item.nombre_completo)}</span><div><strong>{item.nombre_completo}</strong><small>{item.numero_documento}</small></div></div></td><td><span className="op-clamp-2">{item.cargo_nombre ?? "Sin cargo"}</span></td><td><div className="op-assignment-cell">{item.asignacion_actual.institucion ? <><strong>{item.asignacion_actual.institucion}</strong>{item.asignacion_actual.sede && <small>{item.asignacion_actual.sede}</small>}{item.asignacion_actual.municipio && <small>{item.asignacion_actual.municipio}</small>}{item.asignacion_actual.modalidad && <span className="op-assignment-status">Cobertura sí</span>}</> : <><strong>{item.asignacion_actual.nombre ?? "Sin ubicación laboral"}</strong><small>Personal administrativo</small></>}</div></td><td>{item.asignacion_actual.modalidad ? <span className="op-badge status-info">{item.asignacion_actual.modalidad}</span> : <span className="op-muted">—</span>}</td><td><div className="op-offer-cell"><span className={`op-badge ${item.presentada_licitacion_actual ? "status-activa" : "status-suspendida"}`}>{item.presentada_licitacion_actual ? "PRESENTADA" : "NO PRESENTADA"}</span>{item.presentada_licitacion_actual && item.perfil_licitacion_actual && <small title={item.perfil_licitacion_actual}>{item.perfil_licitacion_actual}</small>}</div></td><td><span className={`op-badge status-${item.estado_vinculacion.toLowerCase()}`}>{getStatusLabel(item.estado_vinculacion)}</span></td><td>{formatDate(item.fecha_ingreso)}</td><td className="op-last-updated">—</td><td><button type="button" className="op-link-button" onClick={(event) => { event.stopPropagation(); setSelectedVinculacionId(item.vinculacion_id); }}>Ver expediente <ArrowRight size={14} /></button></td><td><button type="button" className="op-action-button" aria-label={`Abrir acciones de ${item.nombre_completo}`} onClick={(event) => { event.stopPropagation(); setSelectedVinculacionId(item.vinculacion_id); }}>...</button></td>
            </tr>)}
