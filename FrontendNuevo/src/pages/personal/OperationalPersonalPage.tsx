...
export default function OperationalPersonalPage() {
  ...
  useEffect(() => {
    ...
    void getPersonalResumen({ contrato_id: contratoId, fecha: fechaConsulta })
      .then((summary) => {
        if (!cancelled) setPersonalResumen(summary);
      })
      .catch(() => {
        if (!cancelled) setPersonalResumen(null);
      });
    return () => {
      cancelled = true;
    };
    ...
    async function loadExpediente() {
      ...
      try {
        ...
        if (!cancelled) {
          setSelectedExpediente(null);
          setSelectedError(
            getErrorMessage(error, "No fue posible abrir el expediente.")
          );
        }
      } finally {
        if (!cancelled) {
  ...
  function toggleSelectedVinculacion(vinculacionId: number): void {
    setSelectedVinculacionIds((current) =>
      current.includes(vinculacionId)
        ? current.filter((item) => item !== vinculacionId)
        : [...current, vinculacionId]
    );
  }

  function toggleVisibleSelection(checked: boolean): void {
    const visibleIds = (tableData?.items ?? []).map((item) => item.vinculacion_id);
    setSelectedVinculacionIds((current) => {
      if (checked) {
  ...
  return (
    <div className="op-personal-page">
      <header className="op-page-header">
        ...
        <div className="op-header-actions">
          <button
            ...
            disabled={!contratoId || !canExportPersonal}
          >
          ...
          <button
            ...
            disabled={!contratoId || !canPrepareImport}
          >
      ...
      <section className="op-tools-bar">
        ...
        {showMoreFilters && (
          <div className="op-more-filters-panel">
            <label className="op-filter">
              ...
              >
                <option value="">Todas</option>
                {filterOptions.ubicaciones_laborales.map((item) => (
                  <option key={item.id} value={item.id}>
                    {item.nombre}
                  </option>
                ))}
            ...
            <label className="op-filter">
              ...
              <select
                ...
                onChange={(event) =>
                  setCoberturaFiltro(event.target.value as "" | "SI" | "NO" | "RETIRADA")
                }
                disabled={!contratoId}
              >
                <option value="">Todas</option>
            ...
            <label className="op-filter">
              <span>Oferta</span>
              <select
                value={licitacionFiltro}
                onChange={(event) =>
                  setLicitacionFiltro(
                    event.target.value as "" | "PRESENTADA" | "NO_PRESENTADA"
                  )
                }
                disabled={!contratoId}
              >
        ...
        {(searchValue ||
          ...
          <div className="op-active-filters">
            ...
            {modalidadId && (
              <button type="button" onClick={() => setModalidadId("")}>
                Modalidad ×
              </button>
            )}
            {cargoId && (
              <button type="button" onClick={() => setCargoId("")}>
                Cargo ×
              </button>
            )}
            {estadoFiltro && (
            ...
            <button type="button" className="op-clear-inline" onClick={clearFilters}>
              Limpiar todo
            </button>
          </div>
        )}
        ...
        <div className="op-table-meta">
          <span className="op-count-inline">
            {tableData?.pagination.total
              ? `${(tableData.pagination.personas_total ?? tableData.pagination.total).toLocaleString("es-CO")} trabajadores · ${personalResumen?.trabajadores_activos ?? "—"} activos · ${tableData.pagination.total.toLocaleString("es-CO")} vinculaciones`
              : "0 trabajadores"}
          </span>
          <div className="op-table-meta-actions">
                ...
                <tbody>
                  ...
                  {(tableData?.items ?? []).map((item) => (
                    ...
                    >
                      ...
                      <td className="is-worker">
                        <div className="op-worker-cell">
                          <span className="op-avatar" aria-hidden="true">
                            {getInitials(item.nombre_completo)}
                          </span>
                          <div className="op-worker-copy">
                            <strong>{item.nombre_completo}</strong>
                            <small>{item.numero_documento}</small>
                          </div>
                        </div>
                      </td>
                      <td className="is-role">
                        <span className="op-clamp-2">{item.cargo_nombre ?? "Sin cargo"}</span>
                      </td>
                      ...
                      <td className="is-modality">
                        {item.asignacion_actual.modalidad ? (
                          <span className="op-badge status-info">
                            {item.asignacion_actual.modalidad}
                          </span>
                        ) : (
                        ...
                        <div className="op-offer-cell">
                          ...
                          {item.presentada_licitacion_actual &&
                            item.perfil_licitacion_actual && (
                              <small title={item.perfil_licitacion_actual}>
                                {item.perfil_licitacion_actual}
                              </small>
                            )}
                        </div>
      ...
      {showExportModal && contratoId && currentFilters && (
        <PersonalExportModal
          contratoId={contratoId}
          filters={currentFilters}
          selectedVinculacionIds={selectedVinculacionIds}
