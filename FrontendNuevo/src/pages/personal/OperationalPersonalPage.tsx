...
return (
    <div className="op-personal-page">
      <header className="op-page-header"><div><div className="op-eyebrow"><Users size={16} /> Operación de personal</div><div className="op-title-row"><h1>Personal</h1><label className="op-contract-compact"><FileText size={14} /><span>Contrato</span><select value={contratoId ?? ""} onChange={(event) => { setContratoId(event.target.value ? Number(event.target.value) : null); setSelectedVinculacionId(null); setSelectedExpediente(null); setSelectedError(""); }} disabled={!empresaId}><option value="">Seleccionar</option>{contratos.map((contrato) => <option key={contrato.id} value={contrato.id}>{contrato.numero_contrato}</option>)}</select></label></div><p>Gestiona y consulta la información de todos los trabajadores.</p></div><div className="op-header-actions"><button type="button" className="op-button secondary" onClick={() => setShowImportModal(true)} disabled={!contratoId || !canImportPersonal}><Upload size={15} /> Importar</button><button type="button" className="op-button primary" onClick={() => navigate(buildManagementUrl(true))} disabled={!contratoId || !canCreateVinculacion}><UserPlus size={15} /> Nuevo trabajador</button></div></header>
      <section className="op-kpi-grid" aria-label="Resumen de personal"><article className="op-kpi"><span>Trabajadores activos</span><strong>{personalResumen?.trabajadores_activos ?? "—"}</strong><small>Vigentes al {formatDate(fechaConsulta)}</small></article><article className="op-kpi"><span>Ingresos del mes</span><strong>{personalResumen?.ingresos_mes ?? "—"}</strong><small>Inicios dentro del mes</small></article><article className="op-kpi"><span>Retiros del mes</span><strong>{personalResumen?.retiros_mes ?? "—"}</strong><small>Fechas fin dentro del mes</small></article><article className="op-kpi"><span>Vacantes / cargos sin cubrir</span><strong>{personalResumen?.vacantes ?? "—"}</strong><small>Déficit territorial agregado</small></article></section>
      <section className="op-tools-bar"><div className="op-search-row"><label className="op-search" aria-label="Buscar por documento o nombre"><Search size={15} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nombre, documento o institución..." /></label><button type="button" className={`op-button secondary op-more-filters ${showMoreFilters ? "is-open" : ""}`} onClick={() => setShowMoreFilters((current) => !current)} aria-expanded={showMoreFilters}>Más filtros{activeFilterCount > 0 && <b>{activeFilterCount}</b>}</button></div><div className="op-primary-filters"><label className="op-filter"><span>Municipio</span><select value={municipioId} onChange={(event) => { setMunicipioId(event.target.value); setInstitucionId(""); setSedeId(""); setModalidadId(""); }} disabled={!contratoId}><option value="">Todos</option>{filterOptions.municipios.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label><label className="op-filter"><span>Institución</span><select value={institucionId} onChange={(event) => { setInstitucionId(event.target.value); setSedeId(""); setModalidadId(""); }} disabled={!contratoId || !municipioId}><option value="">Todas</option>{filterOptions.instituciones.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label><label className="op-filter"><span>Sede</span><select value={sedeId} onChange={(event) => { setSedeId(event.target.value); setModalidadId(""); }} disabled={!contratoId || !institucionId}><option value="">Todas</option>{filterOptions.sedes.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label><label className="op-filter"><span>Modalidad</span><select value={modalidadId} onChange={(event) => setModalidadId(event.target.value)} disabled={!contratoId || !sedeId}><option value="">Todas</option>{filterOptions.modalidades.map((item) => <option key={item.id} value={item.id}>{item.codigo ?? item.nombre}</option>)}</select></label><label className="op-filter"><span>Cargo</span><select value={cargoId} onChange={(event) => setCargoId(event.target.value)} disabled={!contratoId}><option value="">Todos</option>{cargos.map((cargo) => <option key={cargo.id} value={cargo.id}>{cargo.label}</option>)}</select></label><label className="op-filter"><span>Estado</span><select value={estadoFiltro} onChange={(event) => setEstadoFiltro(event.target.value as "" | VinculacionEstado)} disabled={!contratoId}><option value="">Todos</option><option value="ACTIVA">Activa</option><option value="SUSPENDIDA">Suspendida</option><option value="RETIRADA">Retirada</option></select></label></div>{showMoreFilters && <div className="op-more-filters-panel"><label className="op-filter"><span>Ubicación laboral</span><select value={ubicacionId} onChange={(event) => setUbicacionId(event.target.value)} disabled={!contratoId}><option value="">Todas</option>{filterOptions.ubicaciones_laborales.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label><label className="op-filter"><span>Cobertura</span><select value={coberturaFiltro} onChange={(event) => setCoberturaFiltro(event.target.value as "" | "SI" | "NO" | "RETIRADA")} disabled={!contratoId}><option value="">Todas</option><option value="SI">Con cobertura</option><option value="NO">Sin cobertura</option><option value="RETIRADA">Retirada</option></select></label><label className="op-filter"><span>Oferta</span><select value={licitacionFiltro} onChange={(event) => setLicitacionFiltro(event.target.value as "" | "PRESENTADA" | "NO_PRESENTADA")} disabled={!contratoId}><option value="">Todas</option><option value="PRESENTADA">Presentada</option><option value="NO_PRESENTADA">No presentada</option></select></label><label className="op-filter"><span>Fecha de consulta</span><input type="date" value={fechaConsulta} onChange={(event) => setFechaConsulta(event.target.value)} disabled={!contratoId} /></label></div>}{(searchValue || municipioId || institucionId || sedeId || modalidadId || cargoId || ubicacionId || coberturaFiltro || licitacionFiltro || estadoFiltro) && <div className="op-active-filters"><span>Filtros activos</span>{searchValue && <button type="button" onClick={() => setSearch("")}>Búsqueda: {searchValue} ×</button>}{municipioId && <button type="button" onClick={() => { setMunicipioId(""); setInstitucionId(""); setSedeId(""); setModalidadId(""); }}>Municipio ×</button>}{institucionId && <button type="button" onClick={() => { setInstitucionId(""); setSedeId(""); setModalidadId(""); }}>Institución ×</button>}{sedeId && <button type="button" onClick={() => { setSedeId(""); setModalidadId(""); }}>Sede ×</button>}{modalidadId && <button type="button" onClick={() => setModalidadId("")}>Modalidad ×</button>}{cargoId && <button type="button" onClick={() => setCargoId("")}>Cargo ×</button>}{estadoFiltro && <button type="button" onClick={() => setEstadoFiltro("")}>Estado ×</button>}<button type="button" className="op-clear-inline" onClick={clearFilters}>Limpiar todo</button></div>}</section>
      <section className="op-table-card"><div className="op-table-meta"><span className="op-count-inline">{tableData?.pagination.total ? `${tableData.pagination.total.toLocaleString("es-CO")} vinculaciones` : "0 vinculaciones"}</span><label className="op-page-size"><span>Filas</span><select value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))} disabled={!contratoId || tableLoading}>{PAGE_SIZE_OPTIONS.map((option) => <option key={option} value={option}>{option}</option>)}</select></label></div>{!contratoId ? <div className="op-empty">Selecciona un contrato para abrir la vista operativa.</div> : tableError ? <div className="op-state error"><AlertTriangle size={16} />{tableError}</div> : tableLoading && !tableData ? <div className="op-empty">Cargando personal del contrato...</div> : <><div className="op-table-scroll"><table className="op-table"><thead><tr><th className="is-worker">Trabajador</th><th className="is-role">Cargo</th><th className="is-assignment">Asignación</th><th className="is-modality">Modalidad</th><th className="is-offer">Oferta</th><th className="is-status">Estado</th><th className="is-date">Ingreso</th><th className="is-action">Expediente</th></tr></thead><tbody>{(tableData?.items ?? []).length === 0 && <tr><td colSpan={8} className="op-empty-row">No hay personal vinculado a este contrato con los filtros actuales.</td></tr>}{(tableData?.items ?? []).map((item) => <tr key={item.vinculacion_id} className={item.vinculacion_id === selectedVinculacionId ? "is-selected" : ""} onClick={() => setSelectedVinculacionId(item.vinculacion_id)}><td><div className="op-worker-cell"><strong>{item.nombre_completo}</strong><small>{item.numero_documento}</small></div></td><td><span className="op-clamp-2">{item.cargo_nombre ?? "Sin cargo"}</span></td><td><div className="op-assignment-cell">{item.asignacion_actual.institucion ? <><strong>{item.asignacion_actual.institucion}</strong>{item.asignacion_actual.sede && <small>{item.asignacion_actual.sede}</small>}{item.asignacion_actual.municipio && <small>{item.asignacion_actual.municipio}</small>}{item.asignacion_actual.modalidad && <span className="op-assignment-status">Cobertura sí</span>}</> : <><strong>{item.asignacion_actual.nombre ?? "Sin ubicación laboral"}</strong><small>Personal administrativo</small></>}</div></td><td>{item.asignacion_actual.modalidad ? <span className="op-badge status-info">{item.asignacion_actual.modalidad}</span> : <span className="op-muted">—</span>}</td><td><div className="op-offer-cell"><span className={`op-badge ${item.presentada_licitacion_actual ? "status-activa" : "status-suspendida"}`}>{item.presentada_licitacion_actual ? "PRESENTADA" : "NO PRESENTADA"}</span>{item.presentada_licitacion_actual && item.perfil_licitacion_actual && <small title={item.perfil_licitacion_actual}>{item.perfil_licitacion_actual}</small>}</div></td><td><span className={`op-badge status-${item.estado_vinculacion.toLowerCase()}`}>{getStatusLabel(item.estado_vinculacion)}</span></td><td>{formatDate(item.fecha_ingreso)}</td><td><button type="button" className="op-link-button" onClick={(event) => { event.stopPropagation(); setSelectedVinculacionId(item.vinculacion_id); }}>Ver expediente <ArrowRight size={14} /></button></td></tr>)}</tbody></table></div><div className="op-pagination"><span>{tableData?.pagination.total ? `${(tableData.pagination.page - 1) * tableData.pagination.limit + 1} - ${Math.min(tableData.pagination.page * tableData.pagination.limit, tableData.pagination.total)} de ${tableData.pagination.total}` : "Sin resultados"}</span><div className="op-pagination-actions"><button type="button" className="op-button secondary" disabled={!tableData || tableData.pagination.page <= 1 || tableLoading} onClick={() => setPage((current) => Math.max(1, current - 1))}>Anterior</button><button type="button" className="op-button secondary" disabled={!tableData || tableData.pagination.page >= tableData.pagination.total_pages || tableLoading} onClick={() => setPage((current) => current + 1)}>Siguiente</button></div></div></>}</section>
      {showImportModal && contratoId && empresaActiva && contratoSeleccionado && <OperationalImportModal contratoId={contratoId} empresaNombre={empresaActiva.nombre_empresa} contratoNombre={contratoSeleccionado.numero_contrato} canConfirm={canConfirmImport} onClose={() => setShowImportModal(false)} onImported={() => setRefreshIndex((current) => current + 1)} />}{selectedVinculacionId !== null && <div className="op-drawer-layer" onClick={closeDrawer}><aside className="op-drawer" onClick={(event) => event.stopPropagation()}><PersonalMasterDrawer expediente={selectedExpediente} loading={selectedLoading} error={selectedError} onClose={closeDrawer} onOpenManagement={() => navigate(buildManagementUrl(false))} onRefresh={() => setRefreshIndex((current) => current + 1)} permissions={permissions} tipoDocumentoOptions={tiposDocumento} tipoIdentificacionOptions={tiposIdentificacion} /></aside></div>}
    </div>
  );}
