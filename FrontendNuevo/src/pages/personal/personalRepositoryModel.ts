...
const specs: Spec[] = [
 ...
 ['DOTACION','SST','Dotación',false,[],true], ['EPP','SST','Entrega EPP',false,[],true],
...
export function canonicalColumns(catalog: CatalogoItem[]): MatrixItem[] { return specs.map(([code,group,label,required,aliases,multi]) => { const matches=catalog.filter(t=>aliases.includes(t.codigo??'')); const identity=code==='IDENTIDAD'?matches.map(t=>t.id):undefined; return {proceso:code==='DOTACION'||code==='EPP',canonical_code:code,group,label,ambito_documental:'PERSONA',tipo_documento_id:matches[0]?.id??null,tipo_documento_ids:matches.map(t=>t.id),identity_type_ids:identity,codigo:code,nombre_requisito:label,tipo_documento_nombre:label,documento_id:null,fuente_documento:null,estado_detallado:required?'SIN_DOCUMENTO':'ACREDITABLE',cuenta_cumplimiento:required,multi_record:multi,component_codes:code==='MANIPULACION'?['CURSO','EXAMENES']:undefined,requiere_fecha_expedicion:matches.some(t=>t.requiere_fecha_expedicion),requiere_fecha_vencimiento:matches.some(t=>t.requiere_fecha_vencimiento)}; }); }
export function repositoryCell(row: RepositoryRow, column: MatrixItem): MatrixItem {
  ...
  const requirements = row.checklist?.requisitos.filter(item => item.codigo === column.canonical_code ||
  ...
  return { ...column, documento_id: source?.documento_id ?? docs[0]?.documento_id ?? null,
