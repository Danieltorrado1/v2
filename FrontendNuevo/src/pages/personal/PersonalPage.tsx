...
function EditarEmpleadoModal({
  ...
  const [municipio,setMunicipio]=useState(''),[institucion,setInstitucion]=useState(''),[sede,setSede]=useState(''),[modalidad,setModalidad]=useState('');
  const [fechaEfectiva,setFechaEfectiva]=useState(todayIso());
  const [institutionSearch,setInstitutionSearch]=useState(''),[siteSearch,setSiteSearch]=useState('');
  useEffect(()=>{if(!vinculacion)return;void getOperativeAssignmentOptions(vinculacion.id).then(rows=>{setOptions(rows);const current=rows.find(x=>x.municipio===expediente?.asignacion_actual.municipio&&x.institucion===expediente?.asignacion_actual.institucion&&x.sede===expediente?.asignacion_actual.sede&&x.modalidad===expediente?.asignacion_actual.modalidad);if(current){setMunicipio(current.municipio_id);setInstitucion(current.institucion_id);setSede(current.sede_id);setModalidad(current.modalidad_id)}}).catch(err=>setApiError(err instanceof Error?err.message:'No fue posible cargar la asignación operativa'))},[vinculacion,expediente]);
  useEffect(()=>{const current=expediente?.personal_contexto.asignacion_operativa_actual;if(!current||options.length===0)return;const option=options.find(item=>Number(item.id)===current.focalizacion_final_id);if(option){setMunicipio(option.municipio_id);setInstitucion(option.institucion_id);setSede(option.sede_id);setModalidad(option.modalidad_id)}},[expediente,options]);
  ...
  const modalities=Array.from(new Map(options.filter(x=>x.institucion_id===institucion&&x.sede_id===sede).map(x=>[x.modalidad_id,x.modalidad])).entries());
    ...
    <div style={MODAL_OVERLAY} onClick={onClose}>
      <div style={MODAL_BOX} onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={(e) => { void handleSubmit(e); }}>
          ...
          {vinculacion && <label style={FIELD_LABEL}>Fecha efectiva del cambio<input type="date" required style={FIELD_INPUT} value={fechaEfectiva} onChange={e=>setFechaEfectiva(e.target.value)} /><small style={{fontSize:11,color:'var(--text-secondary)'}}>Use una fecha histórica para corregir un periodo abierto.</small></label>}
          {apiError && <FormError msg={apiError} />}
