...
export function RoleModuleVisibility() {
  ...
  const choose = (role: string) => setConfig((getRoleConfig(role, empresaId ?? 0) ?? { modules: Object.fromEntries(entries.map((item) => [item.code, role === 'TALENTO_HUMANO' ? item.code === 'PERSONAL' || item.code === 'NOMINA' : true])), children: Object.fromEntries(entries.flatMap((item) => item.children.map((child) => [child.code, true]))) }));
