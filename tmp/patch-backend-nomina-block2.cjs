...
let service = fs.readFileSync('src/modules/empresa-configuracion/empresa-configuracion.service.ts', 'utf8');
service = replaceOrThrow(service,
...
`, 'options row');
service = replaceOrThrow(service, 'const buildAssignmentPreviewQuery = (', 'export const buildAssignmentPreviewQuery = (', 'export query');
service = replaceOrThrow(service, 'const loadTargetCategoryForPreview = async (', `const intersectsDateRange = (
...
const loadTargetCategoryForPreview = async (`, 'helper insertion');
service = replaceOrThrow(service,
...
`    for (const row of changedRows) {
      ...
      const before = {
`, 'assignment action');
service = service.replace(/      await registerAuditEntry\({[\s\S]*?        ip: auditMeta\?\.ip \?\? null,\n        user_agent: auditMeta\?\.user_agent \?\? null\n      }\);/, `      await registerAuditEntry({
        client,
...
if (!service.includes('criterios_preview: criteriaSnapshot')) throw new Error('audit enrichment failed');
fs.writeFileSync('src/modules/empresa-configuracion/empresa-configuracion.service.ts', service);

let routes = fs.readFileSync('src/modules/empresa-configuracion/empresa-configuracion.routes.ts', 'utf8');
routes = replaceOrThrow(routes,
...
fs.writeFileSync('src/modules/empresa-configuracion/empresa-configuracion.routes.ts', routes);
